...
if ($space_id <= 0 || !$date || !$time_start || !$time_end) {
    header('Location: ' . url('/admin.php?create_error=Preencha todos os campos'));
    exit;
}
...
try {
    ...
    header('Location: ' . url('/admin.php?create_success=Slot criado'));
    exit;
} catch (Throwable $e) {
    header('Location: ' . url('/admin.php?create_error=' . urlencode('Erro: '.$e->getMessage())));
    exit;
}
